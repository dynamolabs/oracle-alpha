    ...
    for (const signalAccount of signals) {
      ...
      if (currentPrice > signal.athPrice.toNumber()) {
        try {
          ...
          updatedCount++;
          console.log(`[ATH] Updated ${signal.symbol}: ${signal.athPrice.toNumber()} -> ${currentPrice}`);
        } catch (e: any) {
      ...
      if (drawdownFromAth > 0.5 || drawdownFromEntry > 0.3) {
        ...
        const roiBps = entryPrice > 0 
          ? Math.floor(((currentPrice - entryPrice) / entryPrice) * 10000)
          : 0;
